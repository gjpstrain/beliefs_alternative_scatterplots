...
lowest summed standard deviation, which in this case is statements 2 and
22 (see Table~\ref{tbl-pre-test-hi} and Table~\ref{tbl-pre-test-low}).
In order to control for statement directionality, the wording of
statement 22 is changed; the final statements can be see in
Section~\ref{sec-main-study}.

\subsection{Discussion}\label{sec-discussion-pre}

Fleiss' Kappa values for interrater agreement on both topic emotionality
and strength of correlation scales are low ((\(\kappa\) = 0.07
and\(\kappa\) = 0.06 respectively), however do exceed that which would
be expected by chance. We suggest this may be due to Fleiss' Kappa not
being designed with ordinal (Likert scales in this case) data in mind.
In light of this we do not make decisions regarding which statements to
